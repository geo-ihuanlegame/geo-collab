...
- 将题目快照渲染成编号问题列表。
- 如果模板包含 `{{问题}}`，替换该占位符（高级用法：产品自定问题插入位置）。
- 如果模板不包含 `{{问题}}`，把编号问题块**前置**到模板正文之前（桥接句带问题条数 N）：

```text
基于以下 2 个问题，结合参考这些问题生成 1 篇文章：
...
- 题目快照为空时只用模板正文，不插入空问题块。

> 产品用法：把 docx 里的提示词主体复制进「提示词管理」即可，**删掉**文档开头写死的示例问题；普通产品同事**不需要**写 `{{问题}}`，真实提问词由系统自动前置。渲染逻辑单一来源在 `article_writer.render_question_prompt`，旧 `pipeline.py` 直接复用同一函数。
