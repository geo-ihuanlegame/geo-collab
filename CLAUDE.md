...
POC 期：`server/mcp/` 跑独立 Python 进程（FastMCP stdio），把 GEO 现有 + 新增 API 包装成 ~17 个 atomic tools 给 Claude Code 调用。Loop 配方在 `claude-loops/*.md`。
...
- 生产启用加密的顺序固定：① `alembic upgrade head`（迁移把 `api_credentials`/`api_token_cache` 两列 JSON→TEXT）→ ② 部署带密钥的新代码 →
  ③ `python -m server.scripts.encrypt_secrets` 幂等回填。读路径全程认 `enc:v1:` 前缀、无前缀＝明文，
  故迁移期混存安全、零停机。`encrypt_secrets` 可重跑。web 进程与发布 worker（`server/worker/executor.py`）**必须配置相同的 `GEO_SECRET_KEY`/`GEO_SECRET_KEYS`** —— worker 发布时要解密 `api_credentials`/`api_token_cache`，密钥不一致会导致 worker 解密失败（RuntimeError）或读到错误数据。回滚（`alembic downgrade` 到迁移前版本）前必须先把两列解密回明文——TEXT→JSON 会拒绝 `enc:v1:` 密文串（非法 JSON）。
