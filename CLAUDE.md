...
- 配置：`pydantic-settings`，前缀 `GEO_`。`get_settings()` 走 `@lru_cache`，测试改环境后要 `get_settings.cache_clear()`。运行时改环境变量 / `.env` 后，admin 调 `POST /api/system/refresh-settings` 刷新缓存（Task 1c：`ai_format` / `baidu` 不再每次调用都 `cache_clear`；**前提 web 单进程**，多进程下只刷到接到请求的那个进程）。
...
- AI Key（`GEO_AI_API_KEY` / `GEO_AI_FORMAT_API_KEY`）**启动时不校验**，缺 / 错时在调用 LiteLLM 的请求里才报错。中途改 Key 后调 `POST /api/system/refresh-settings`（admin）即时生效，无需重启（Task 1c）。
